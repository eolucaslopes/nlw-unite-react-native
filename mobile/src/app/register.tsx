...
export default function Register() {
  ...
  async function handelRegister() {
    try {
      ...
      const resgisterResponse = await api.post(`/events/${EVENT_ID}/attendees`, { name, email })

      if (resgisterResponse.data.attendeeID) {
  ...
  return (
    <View className="flex-1 bg-green-500 items-center justify-center p-8">
      ...
      <View className="w-full mt-12 gap-3">
        ...
        <Button title="Realizar inscrição" onPress={handelRegister} isLoading={isLoading} />

        <Link href="/" className="text-gray-100 text-base font-bold
        text-center mt-8">
          Já possui ingresso?
        </Link>
      </View>
